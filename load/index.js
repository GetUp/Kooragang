...
const report = async () => {
  ...
  const wait = waitEvents.length ? Math.round(_.sumBy(waitEvents,
    event => JSON.parse(event.value).seconds_waiting) / waitEvents.length) : 0
...
const addAgent = async (count) => {
  ...
  for (let _step of range) {
    const agent = 61400000001 + agents++
    const params = {
      to: selectTarget(),
      from : agent,
