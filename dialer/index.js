/* eslint no-empty: 0, no-await-in-loop: 0, no-restricted-syntax: 0 */
...
module.exports.dial = async (appUrl, campaignBeforeRatioRecalc) => {
  const timer = new Date()
  const campaign = await recalculateRatio(campaignBeforeRatioRecalc)
  ...
  if (callsToMakeExcludingCurrentCalls > 0) {
    ...
    try {
      ...
      await Event.query().insert({ campaign_id: campaign.id, name: 'callee_error', value: { error } })
    }
  }
...
const updateAndCall = async (campaign, callee, appUrl) => {
  ...
  try {
    ...
    await Event.query().insert({ campaign_id: campaign.id, name: 'api_error', value: { calls_in_progress: campaign.calls_in_progress, callee_id: callee.id, error } })
  }
}
...
module.exports.calledEveryone = async campaign => {
  ...
  const { count } = await Callee.query()
    .count('id as count')
    .where({ campaign_id: campaign.id })
    .whereNull('last_called_at')
    .first()
