...
app.get('/api/campaigns/:id/statistics', wrap(async (req, res, next) => {
  ...
  const generateReport = async () => {
    ...
    const tech_issues_reported = _.sumBy(techIssues, ({count}) => parseInt(count, 10));
    const waitEvents = await Event.query()
      .whereIn('name', ['caller_complete', 'answered'])
      .where({campaign_id: campaign.id})
      .whereRaw("created_at >= NOW() - INTERVAL '10 minutes'");
    const average_wait_time = waitEvents.length ? Math.round(_.sumBy(waitEvents, event => JSON.parse(event.value).seconds_waiting) / waitEvents.length) : 0;
    const total_calls = _.sumBy(statusCounts, ({count}) => parseInt(count, 10));
    ...
    data.approximate_rate = currentCallers ? Math.round(total*6 / currentCallers) : 0;
    return data;
  }
