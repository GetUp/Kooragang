...
app.post('/connect', async ({ body, query }, res) => {
  ...
  if (!assessment && (campaign.isPaused() || campaign_is_complete) && campaign.next_campaign_id) {
    ...
    if (next_campaign && (await next_campaign.isOperational())) {
      ...
      _.times(4, () => {
        r.addSpeakI18n(message, {
          campaign_name: campaign.name, next_campaign_name: next_campaign.name, next_campaign_number
        })
        r.addWait({ length: 5 })
      })
      r.addHangup()
  ...
  const reached_dial_in_number_channel_limit = await campaign.reached_dial_in_number_channel_limit(
    dial_in_number, sip_header_present
  )
...
app.post('/transfer_to_target', async ({ query, body }, res) => {
  ...
  const campaign_target_number = _.isArray(campaign.target_numbers) ?
    _.sample(campaign.target_numbers) : campaign.target_numbers
...
app.post('/survey_multiple', async ({ query }, res) => {
  ...
  const matched_previous_response_keys = _.remove(_.map(questionData.answers,
    (answer, key) => _.includes(previous_survey_result_answers, answer.value) ? key : null), null)
