...
app.post('/connect', async ({body, query}, res) => {
  ...
  if (!assessment && (campaign.isInactive() || campaign_is_complete) && campaign.next_campaign_id) {
    ...
    if (next_campaign && (await next_campaign.isOperational())) {
      ...
      if (campaign_is_complete) {
        ...
        r.addSpeakI18n('campaign_status_inactive_with_next', {campaign_name: campaign.name, next_campaign_name: next_campaign.name});
      }
  ...
  if (!assessment && (campaign.isInactive() || campaign_is_complete)) {
    r.addWait({length: 2});
    r.addSpeakI18n('welcome', {org_name: process.env.ORG_NAME || ''});
    r.addWait({length: 1});
    if (campaign_is_complete) {
      ...
      r.addSpeakI18n('campaign_status_inactive', {campaign_name: campaign.name});
    }
    return res.send(r.toXML());
  }
