...
app.post('/redirect', async ({body, query}, res, next) => {
  ...
  if (!campaign.target_number) return next(`No target number set on campaign: ${campaign.id}`);
  ...
  await Redirect.query().insert({
    campaign_id: campaign.id, phone_number: body.From, redirect_number: body.To,
    target_number: campaign.target_number, call_uuid: body.CallUUID,
