...
module.exports = async (fields, area) => {
  ...
  const [code1, res1] = await plivo_api('create_application', payload, {multiArgs: true})
  const app_id = res1.app_id
  const [code2, res2] = await plivo_api('search_phone_numbers', area, {multiArgs: true})
  ...
  if (campaign.target_number) {
    ...
    const [code3, res4] = await plivo_api('create_application', redirect_payload, {multiArgs: true})
    const redirect_app_id = res4.app_id
    await plivo_api('buy_phone_number', {number: redirect_number, app_id: redirect_app_id})
